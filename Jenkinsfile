  ...
  """
      ...
      node (label) {
          ...
          stage ('Checkout SCM'){
            git credentialsId: 'git', url: 'https://github.com/prpd1/eos-gateway-api-deploy.git', branch:  "${env}"
          }
          ...
          stage ('Helm Chart') {
            container('build') {
                withCredentials([usernamePassword(credentialsId: 'jfrog', usernameVariable: 'username', passwordVariable: 'password')]) {
                      ...
                      sh "/usr/local/bin/helm upgrade  --install --force gateway-api  --namespace ${env} -f values.yaml eos-helm-helm-local/gateway-api"
